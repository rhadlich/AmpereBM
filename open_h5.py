...
dir = os.getcwd()
filedir = os.path.join(dir, 'ampere_bm.h5')

with h5py.File(filedir, 'a') as f:
    for key in f.keys():
        grp = f[key]
        params_list = []
        for i, key2 in enumerate(grp.keys()):
            params_list.append({})
            subgroup = grp[key2]
            for j, key3 in enumerate(subgroup.keys()):
                params_list[i]['Processes'] = int(key2)
                params_list[i][key3] = subgroup[key3][...]
            print(params_list)
print(params_list[0]['end time'][0].decode('utf-8'))
#     # Small Model
#     # Make dataframe
#     proc_per_node = 16
#     grp1 = f['small']
